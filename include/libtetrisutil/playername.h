...
/** #47 decision 2's cap, and MAX_PLAYER_NAME - 1: the username IS the roster
 * display name, so the two move together. */
#define PLAYER_NAME_MAX (MAX_PLAYER_NAME - 1)
...
/**
 * Is this a legal player name: the allowlist, 1..PLAYER_NAME_MAX characters.
 *
 ...
 */
bool player_name_ok(const char *s, size_t len);
 ...
 * @param cap  Capacity of dst; must be at least MAX_PLAYER_NAME.
 * @param s    Name; need not be NUL-terminated.
 * @param len  Length in bytes.
