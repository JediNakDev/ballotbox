...
void test_U09_election_not_found(void) {
  ...
  TEST_ASSERT_EQUAL_INT(BB_ERR_NOT_FOUND, bb_join(ctx, "E-999", "alice", &out));
...
void test_U10_election_not_open(void) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    TEST_ASSERT_EQUAL_INT_MESSAGE(BB_ERR_NOT_OPEN, bb_join(ctx, "E-100", "alice", NULL),
...
void test_U11_unlisted_cert_refused(void) {
  ...
  TEST_ASSERT_EQUAL_INT(BB_ERR_NOT_ELIGIBLE, bb_join(ctx, "E-100", "mallory", &out));
  TEST_ASSERT_EQUAL_INT(0, fake_write_count());
...
void test_U12_invalid_or_expired_cert_refused(void) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    TEST_ASSERT_EQUAL_INT(expected[i], bb_join(ctx, "E-100", "alice", NULL));
...
void test_U13_eligible_voter_admitted(void) {
  ...
  bb_election_t out;
  memset(&out, 0, sizeof(out));
  TEST_ASSERT_EQUAL_INT(BB_OK, bb_join(ctx, "E-100", "alice", &out));
  ...
  TEST_ASSERT_EQUAL_INT(BB_OK, bb_join(ctx, "E-100", "bob", NULL));
}
...
void test_cert_is_checked_before_state(void) {
  ...
  TEST_ASSERT_EQUAL_INT(BB_ERR_CERT_INVALID, bb_join(ctx, "E-100", "alice", NULL));
}
...
void test_store_failure_is_propagated(void) {
  ...
  TEST_ASSERT_EQUAL_INT(BB_ERR_DB, bb_join(ctx, "E-100", "alice", NULL));
}
