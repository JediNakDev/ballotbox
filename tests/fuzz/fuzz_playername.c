/*
 * fuzz_playername.c - player_name_ok() / player_name_fold() on arbitrary bytes.
 *
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  bool ok = player_name_ok(name, size);
  ...
  if (ok) {
    ...
    char dst[MAX_PLAYER_NAME];
    memset(dst, 0x7F, sizeof dst);
    FUZZ_CHECK(player_name_fold(dst, sizeof dst, name, size) == 0);
    FUZZ_CHECK(fuzz_is_cstr(dst, sizeof dst));
    FUZZ_CHECK(strlen(dst) == size);
    for (size_t i = 0; i < size; i++) {
      unsigned char c = (unsigned char)name[i];
      unsigned char want = (c >= 'A' && c <= 'Z') ? (unsigned char)(c - 'A' + 'a') : c;
      FUZZ_CHECK((unsigned char)dst[i] == want);
    }

    /* Folding is idempotent: the stored form of a stored name is itself. */
    char twice[MAX_PLAYER_NAME];
    FUZZ_CHECK(player_name_fold(twice, sizeof twice, dst, strlen(dst)) == 0);
    FUZZ_CHECK(strcmp(twice, dst) == 0);
  } else {
    /* A rejected name must not be written anywhere: "untouched on failure". */
    char dst[MAX_PLAYER_NAME];
    memset(dst, 0x7F, sizeof dst);
    FUZZ_CHECK(player_name_fold(dst, sizeof dst, name, size) == -1);
    for (size_t i = 0; i < sizeof dst; i++) FUZZ_CHECK(dst[i] == 0x7F);
  }

  /* An undersized destination is refused whatever the name - the cap check
   * must not depend on the name being legal first. */
  if (size >= 1) {
    char small[MAX_PLAYER_NAME];
    FUZZ_CHECK(player_name_fold(small, MAX_PLAYER_NAME - 1, name, size) == -1);
  }
