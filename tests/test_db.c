...
static int with_runner(int (*fn)(void)) {
  ...
  if (rc == 0)
    ...
    fprintf(stderr, "    FAIL: the runner never accepted a connection\n");
...
static int test_socket_queries(void) { return with_runner(conn_queries); }
static int test_socket_transactions(void) { return with_runner(conn_transactions); }
...
int main(void) {
  ...
  run("does not mistake the trailer for a row", test_rows_ignores_the_trailer);
  ...
  if (!runner_disabled() && have_jar() && have_java()) {
    ...
    run("reads rows over a socket and survives a bad statement",
        test_socket_queries);
    run("commits and rolls back across statements", test_socket_transactions);
  } else {
    /*
     ...
     * the jar is gitignored and CI has no JDK step, so these tests have never
     * run on a push, which is this repository proving the point (#54).
     */
