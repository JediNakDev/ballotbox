...
int screen_login(void) {
  for (;;) {
    char name[32] = "";
    if (tui_input("ballotctl - admin login", "Enter admin cert name (Esc to quit):", name, sizeof(name)) != 0) {
      return 0;
    }
...
static int pick_election(const char *title) {
  ...
  return tui_menu(title, items, g_election_count, NULL);
}
...
void screen_create_election(void) {
  char values[5][TUI_FIELD_LEN] = {"", "", "", "2026-01-01 09:00", "2026-01-08 09:00"};
  const char *labels[5] = {"Title", "Options (comma-separated)", "Eligible voter certs (comma-separated)",
                            "Open time", "Close time"};

  for (;;) {
    if (tui_form("Create election (UC-1)", labels, values, 5) != 0) return; /* cancelled */
    ...
    if (strlen(values[0]) == 0 || opt_count == 0) {
      ...
      tui_message("Rejected (alt flow 4a)", lines, 2);
      continue;
    }
...
void screen_election_status(void) {
  ...
  tui_list_view("Election status", lines, g_election_count);
}
