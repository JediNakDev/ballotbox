...
static int block_count(const char *rows) {
  ...
  for (const char *p = rows; p != NULL && *p != '\0'; p = next_line(p), n++) {
    ...
    while (*t >= '0' && *t <= '9') {
      int digit = *t++ - '0';
      /* Guarded BEFORE the multiply: signed overflow is undefined, so
       * checking afterwards is too late to detect - the same reasoning
       * htttp.c's parse_uint gives. A trailer claiming more rows than an int
       * can hold is not a count this body could honestly carry, so it joins
       * every other unreadable trailer at -1 rather than wrapping to a
       * plausible-looking number. Found by tests/fuzz/fuzz_rows. */
