 ...
 *
 * A separate pair from auth.c because two binaries need it: bin/tetrisdb
 ...
 *
 * NO O_CREAT ANYWHERE IN auth_secret_load(). The retry was deleted rather
 ...
 * their next attempt.
 *
 * auth_secret_provision() has exactly one caller (bin/tetrisdb start, inside
...
#define AUTH_SECRET_REL "auth/jwt_secret"
#define AUTH_SECRET_MIN 32
#define AUTH_SECRET_MAX 64
#define AUTH_SECRET_CREATE 32

static int build_path(const char *root, char *buf, size_t buflen)
{
    int n = snprintf(buf, buflen, "%s/" AUTH_SECRET_REL, root);
...
static int validate_fd(int fd, long *out_len, char *defect, size_t defectlen)
{
    ...
    if (st.st_size < AUTH_SECRET_MIN || st.st_size > AUTH_SECRET_MAX)
    {
        snprintf(defect, defectlen, "wrong size (%lld bytes, want %d..%d)",
                 (long long)st.st_size, AUTH_SECRET_MIN, AUTH_SECRET_MAX);
        return -1;
    }
...
int auth_secret_load(const char *root, unsigned char *out, size_t outlen)
{
    ...
    {
        log_send(LOG_DEBUG, "auth/jwt_secret: path too long for root '%s'",
                 root);
        return -1;
    }
    ...
    if (fd < 0)
    {
        log_send(LOG_DEBUG, "%s: %s", path, defect);
        return -1;
    }
    ...
    while (total < (size_t)len)
    {
        ...
        if (n < 0)
        {
            if (errno == EINTR)
                continue;
            log_send(LOG_DEBUG, "%s: read failed: %s", path, strerror(errno));
            close(fd);
            return -1;
        }
        if (n == 0)
        {
            log_send(LOG_DEBUG, "%s: short read (%zu of %ld bytes)", path,
                     total, len);
            close(fd);
            return -1;
        }
...
{
    log_send(LOG_DEBUG, "provisioning authentication secret root=%s", root);
    char path[PATH_MAX];
    char defect[128];
    ...
    if (build_path(root, path, sizeof path) != 0)
    {
        ...
        (void)log_send(
            LOG_INFO,
            "operation=auth_secret_provision phase=complete status=-1");
        return -1;
    }
    ...
    if (fd >= 0)
    {
        ...
        (void)log_send(
            LOG_INFO,
            "operation=auth_secret_provision phase=complete status=0");
        return 0;
    }
    if (!absent)
    {
    ...
    unsigned char key[AUTH_SECRET_CREATE];
