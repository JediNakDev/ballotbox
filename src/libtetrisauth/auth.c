 ...
 * are in include/libtetrisauth/auth.h. Read it before changing a line
 ...
 *auth.h for why the line falls there.
 *
 ...
 *auth_login() that gets moved below the loop or deleted. That is stated
 ...
 *auth_reset_for_test(): a test that wants a fresh counter forks, because
...
static session_t *g_sh; /**< The sessionauth_login() was handed; retained
                          becauseauth_offer() takes no session_t and still
...
static char g_name[MAX_PLAYER_NAME]; /**< Display name AND the guest flag: a
                                       ...
                                       disagree. Read byauth_name(). */

/* Failed credential attempts on this connection: 401 and 404 only, never
 * reset (#56). Onlyauth_login() can move it, because identity is fixed by
...
static void reply(int status, const char *body)
{
    ...
    if (htttp_serialize_response(&res, out, &out_len) == HTTTP_OK)
         ...
         * request: the serializer refuses those silently, so it is logged here
         * or it is invisible. */
 ...
 * by the two method arms ofauth_login()'s loop.
 *
...
{
    log_send(LOG_DEBUG, "authentication request method=%s body_len=%u",
             is_register ? "REGISTER" : "LOGIN", req->body_len);
    ...
    log_send(LOG_DEBUG, "authentication credentials parsed method=%s user=%s",
             is_register ? "REGISTER" : "LOGIN", name);
     ...
     * only on the path where the row is known to exist and the token is known
    ...
    (void)log_send(status >= 500   ? LOG_ERROR
                   : status >= 400 ? LOG_WARN
                                   : LOG_INFO,
                   "operation=credential_flow phase=complete method=%s "
                   "status=%d",
                   is_register ? "REGISTER" : "LOGIN", status);
 ...
 *auth_offer()'s "returns false if the request is none of its business" and
 *auth_login()'s pre-auth gate have to agree on.
 *
...
{
    log_send(LOG_DEBUG, "processing pre-authentication method=%s body_len=%u",
             req->method, req->body_len);
    switch (auth_method_of(req))
    {
    case AUTH_GUEST:
        accept_guest();
        (void)log_send(LOG_INFO,
                       "operation=pre_auth_frame phase=complete method=GUEST "
                       "status=1");
        return true;
    case AUTH_LOGIN:
    {
    ...
    default:
        ...
        (void)log_send(LOG_WARN,
                       "operation=pre_auth_frame phase=complete method=%s "
                       "status=401",
                       req->method);
        return false;
    }
...
int auth_login(session_t *sh)
{
    ...
    OPENSSL_cleanse(buf, sizeof buf);
    (void)log_send(LOG_INFO, "operation=auth_login phase=complete status=%d",
                   verdict);
    return verdict;
}
