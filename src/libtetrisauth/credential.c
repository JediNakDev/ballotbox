 ...
 *
 * Phase 4e (#55). Contract in auth.h; the rules themselves are #47's and
...
{
    /* The allowlist, the length and the fold are libtetrisutil/playername.h's,
     * so this reader and the client's form validator cannot disagree about what
     * a legal name is. */
              ...
              int iters, char *out_hex, size_t cap)
{
    ...
    if (out_hex == NULL || cap < CRED_DIGEST_BYTES * 2 + 1 || iters < 1)
    {
        (void)log_send(LOG_INFO,
                       "operation=cred_hash phase=complete status=-1");
        return -1;
    }
    ...
    (void)log_send(LOG_INFO, "operation=cred_hash phase=complete status=%d",
                   rc);
    return rc;
}
