 ...
 *
 *   tetrisdb start   provision, spawn the JVM and wait for readiness
 *   tetrisdb check   return whether the configured socket is reachable
 *   tetrisdb stop    SIGTERM the runner named by the directory lock
 *
 ...
 * The readiness poll reports only the result of this invocation. Nothing in
 * the system caches runner health.
 */
...
static int provision(const db_runner_opts_t *opts)
{
    ...
    char error[256];
    if (auth_secret_provision(".", error, sizeof error) != 0)
    {
